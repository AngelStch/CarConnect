...
const resturantSignup = async (req, res) => {
  ...
  try {
    ...
    if (isResturantExists) {
      res.status(409).json({
        success: false,
        messsage: "Продавач вече е регистриран с това име",
      });
    } else {
      ...
      res.status(201).json({
        success: true,
        messsage: "Продавачът се регистрира успешно",
        data: resturantData,
      });
    }
  } catch (error) {
    ...
    try {
      if (resturantData) {
        ...
        if (resturantData && hashPassword) {
          res.status(200).json({
            success: true,
            message: "Продавачът се регистрира успешно",
            token: token,
            data:resturantData
          });
        } else {
        ...
        res.status(404).json({
          success: false,
          message: "Продавачът не е регистриран с този имейл",
        });
      }
    } catch (error) {
      res.status(500).json({
