    ...
    if (isCartExists) {
      if (offer.quantity < (isCartExists.quantity + body.quantity)) {
        return res.status(400).json({
          success: false,
          data: {},
          message: "Няма повече свободни автомобили",
        });
      }
    ...
    for (const cartItem of cartItems) {
      ...
      const offer = await offerSchema.findOne({ _id: cartItem.offerId });
       ...
       if (cartItem.quantity > offer.quantity) {
        return res.status(400).json({
          success: false,
          message: `Количеството "${offer.mealName}" надвишава количството свободни коли в обявата.`,
        });
      }
...
const removeFromCart = async (req, res) => {
  ...
  try {
    ...
    if (cartItem) {
      ...
      res.status(200).json({
        success: true,
        message: "Премахнато от количката",
      });
    } else {
      res.status(404).json({
        success: false,
        data: cartItem,
        message: "Не е намерено в количката",
      });
    }
  } catch (error) {
    res.status(500).json({
...
const deleteCartOffer = async (req, res) => {
  try {
    ...
    if (cart) {
      ...
      res.status(200).json({
        success: true,
        messsage: "Количката изтрита",
      });
    } else {
      res.status(200).json({
        success: true,
        messsage: "Не е намерена",
        data: [],
      });
    }
  } catch (error) {
